...
public :
        ...
        ATEXIT_( bool Make_ATEXIT )
	{
	  ...
	  if ( Make_ATEXIT && !ATEXIT_Done ) {
		...
		DESTRUCTEUR_GENERIQUE_::Destructeurs = new list<DESTRUCTEUR_GENERIQUE_*> ; // Destructeurs alloué dynamiquement (cf. ci-dessous) ,
...
void Nettoyage( void )
{
	...
	{
		list<DESTRUCTEUR_GENERIQUE_*>::iterator it = DESTRUCTEUR_GENERIQUE_::Destructeurs->end() ;
...
const int DESTRUCTEUR_GENERIQUE_::Ajout( DESTRUCTEUR_GENERIQUE_ &objet )
{
	...
	//CCRT
	ASSERT(DESTRUCTEUR_GENERIQUE_::Destructeurs) ;
	DESTRUCTEUR_GENERIQUE_::Destructeurs->push_back( &objet ) ;
	return DESTRUCTEUR_GENERIQUE_::Destructeurs->size() ;
}
