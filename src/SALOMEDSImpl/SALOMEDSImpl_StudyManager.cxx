...
static void SaveAttributes(Handle(SALOMEDSImpl_SObject) aSO, HDFgroup *hdf_group_sobject) 
{
  ...
  for(; Itr.More(); Itr.Next()) {
    ...
    if(anAttr->DynamicType() == STANDARD_TYPE(SALOMEDSImpl_AttributeIOR)) continue;  
    if(anAttr->DynamicType() == STANDARD_TYPE(SALOMEDSImpl_AttributeExternalFileDef)) continue;  
    if(anAttr->DynamicType() == STANDARD_TYPE(SALOMEDSImpl_AttributeFileType)) continue; 
