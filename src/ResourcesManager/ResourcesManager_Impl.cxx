...
bool Manager_Impl::ping() {
  MESSAGE("Manager_Impl::ping()") ;
  return true ;
}

bool Manager_Impl::SshAccess( const char * aComputerName ) {
...
//  MESSAGE(endl << "Manager_Impl::GetComputers() search in list of "
...
//  MESSAGE("Manager_Impl::GetComputers() " << aListOfComputers->length() << " found" << endl ) ;
  return aListOfComputers._retn() ;
}
...
Resources::Computer_ptr Manager_Impl::GetComputer( const Resources::ListOfComputers & aListOfComputers ) {
  ...
  int i ;
  for ( i = 0 ; i < aListOfComputers.length() ; i++ ) {
    if ( aListOfComputers[ i ]->IsAlive() &&
         !strcmp( aListOfComputers[ i ]->Alias() , GetHostname().c_str() ) ) {
      aComputer = aListOfComputers[ i ] ;
      break ;
    }
  }
  ...
  if ( CORBA::is_nil( aComputer ) ) {
    MESSAGE("Manager_Impl::GetComputer() : nil_objref_Computer") ;
  }
  else {
    MESSAGE("Manager_Impl::GetComputer() : " << aComputer->Alias() ) ;
  }
