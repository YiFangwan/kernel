...
bool Computer_Impl::IsAlive() {
//  MESSAGE("Computer_Impl::IsAlive()") ;
  bool IsAlive = true ;
  string ping( "ping -c 1 -t 2 -q " ) ;
  if ( strcmp( Alias() , GetHostname().c_str() ) ) {
//CCRT    string ping( "ping -c 1 -w 2 -q " ) ;
    ping += Alias() ;
    int status = system( ping.c_str() ) ;
    if ( status != 0 ) {
      IsAlive = false ;
    }
  }
//  MESSAGE("Computer_Impl::IsAlive() " << ping << " IsAlive " << IsAlive ) ;
  return IsAlive ;
}

bool Computer_Impl::SshAccess() {
//  MESSAGE("Computer_Impl::SshAccess()") ;
  return _ComputerParameters->SshAccess ;
}

bool Computer_Impl::RshAccess() {
//  MESSAGE("Computer_Impl::RshAccess()") ;
  return !_ComputerParameters->SshAccess ;
}

bool Computer_Impl::Interactive() {
//  MESSAGE("Computer_Impl::Interactive()") ;
  return _ComputerParameters->Interactive ;
}

bool Computer_Impl::Batch() {
//  MESSAGE("Computer_Impl::Batch()") ;
  return _ComputerParameters->Batch ;
}

char * Computer_Impl::UserName() {
//  MESSAGE("Computer_Impl::UserName()") ;
  return CORBA::string_dup( _ComputerParameters->UserName ) ;
}

Containers::System Computer_Impl::Os() {
//  MESSAGE("Computer_Impl::Os()") ;
  return _ComputerParameters->Os ;
}

CORBA::Long Computer_Impl::Memory() {
//  MESSAGE("Computer_Impl::Memory()") ;
  return _ComputerParameters->Memory ;
}

CORBA::Long Computer_Impl::Swap() {
//  MESSAGE("Computer_Impl::Swap()") ;
  return _ComputerParameters->Swap ;
}

CORBA::Long Computer_Impl::CpuClock() {
//  MESSAGE("Computer_Impl::CpuClock()") ;
  return _ComputerParameters->CpuClock ;
}

CORBA::Long Computer_Impl::NbProc() {
//  MESSAGE("Computer_Impl::NbProc()") ;
  return _ComputerParameters->NbProc ;
}

CORBA::Long Computer_Impl::NbNode() {
//  MESSAGE("Computer_Impl::NbNode()") ;
  return _ComputerParameters->NbNode ;
}
